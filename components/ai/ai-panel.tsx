      ...
      <div className='p-4 h-full flex flex-col'>
        <div className="flex-1 overflow-y-auto space-y-4">
          {messages.length === 0 && (
            <div className="space-y-3">
              ...
              {suggestions.map(s => <Button key={s} variant="secondary" className="w-full justify-start text-xs h-9 bg-white border-[#D4CFCA] hover:bg-[#EAE6DF]" onClick={() => sendToAi(s)}>{s}</Button>)}
            </div>
          )}
          {messages.map((m, i) => (
            <div key={i} className={cn("p-3 rounded-lg text-sm", m.role === 'user' ? 'bg-[#EAE6DF]' : 'bg-white border border-[#D4CFCA]')}>
              {m.content}
              {m.type === 'card' && <div className="mt-3 p-3 bg-white border border-[#D4CFCA] rounded flex items-center justify-between"><div className='flex items-center gap-2'><FileCheck size={16} />{m.result.name}</div><Button size="sm" variant="secondary">Download</Button></div>}
            </div>
          ))}
        ...
        <div className='flex flex-col gap-2 mt-4'>
          <div className='flex gap-2'>
            ...
            <Button variant="secondary" onClick={() => fileInputRef.current?.click()} size="icon" className="text-[#8B8680]"><Paperclip size={16} /></Button>
          </div>
        </div>
      </div>
