...
export default function ContractPage() {
  ...
  const { user } = useAuth();
  const [project, setProject] = useState<{ description: string; totalAmount: number; status: string } | null>(null);
    ...
    const fetchProject = async () => {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          setProject(data as { description: string; totalAmount: number; status: string });
