  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const pData = pSnap.docs.map(d => ({ id: d.id, ...(d.data() as Omit<Project, 'id'>) } as Project));
        setProjects(pData);
        ...
        if (projectIds.length > 0) {
           ...
           setMilestones(mSnap.docs.map(d => ({ id: d.id, ...(d.data() as Omit<Milestone, 'id'>) } as Milestone)));
        }
      } catch (e) {
        console.error(e);
      } finally {
